...
import { getSentEmails, searchEmails } from '../../api/emails';
...
const columns: Column<Email>[] = [
  ...
  },
  {
    ...
    render: (row) =>
      row.failureReason ? (
        <span
          className="text-xs text-red-400/70 truncate block max-w-[160px] cursor-help"
...
export default function SentEmailsTab() {
  ...
  useEffect(() => {
    ...
    searchEmails(debouncedQuery)
      .then((data) => {
        // Filter to only sent-type emails
        const filtered = data.results.filter((r) =>
          ['SENT', 'FAILED'].includes(r.status),
        );
        setSearchResults(
          filtered.map((r) => ({
            id: r.id,
  ...
  return (
    <div className="space-y-4">
      ...
      <Table
        ...
        emptyDescription="Emails will appear here after they've been sent or failed."
        emptyIcon={
